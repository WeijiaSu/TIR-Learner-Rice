...
for chromosome in range(1,13):
    ...
    for j in ["DTA","DTC","DTH","DTM","DTT"]:
        Family = j
        print(str(chromosome)+": "+j)
        records = list(SeqIO.parse("chr%s_pred_%s_Seq.fa" % (chromosome, Family), "fasta"))
        n = 0
        l = 0
        for rec in records:
            ...
            if (len(tsdDIC) != 0):
                n += 1
                l += len(tsdDIC)
                Re_writeSeq(tsdDIC, rec, "chr%s_%s_allCheckPre.fa" %(chromosome, Family), TIRlength, Family)
        print(n)
        print(l)
