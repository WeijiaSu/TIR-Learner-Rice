...
def GetFastaFromFile(argsList):
    ...
    outname=genomeName+spliter+"M3F200.fa"
...
def getLTR(rec):
    ...
    blast = "blastn -query %s -subject %s -outfmt '7 qseqid sseqid length pident gaps mismatch qstart qend sstart send evalue qcovhsp' -out %s" % (
    "seq1_" + ID + ".fasta", "seq2_" + ID + ".fasta", ID + "_blast.tsv")
    os.system(blast)
...
if __name__ == '__main__':
    records = list(SeqIO.parse(genome_Name+spliter+"M3F200.fa", "fasta"))
    pool = multiprocessing.Pool(int(t))
...
cat = "cat *.tsv > %sModule3%sLow"%(genome_Name+spliter,spliter)
os.system(cat)
rm = "rm *.tsv"
os.system(rm)
rm ="rm *predi*DT*.fa"
