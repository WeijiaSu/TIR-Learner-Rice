...
def CheckTSD(rec):
    ...
    last20=s[-2:]
    s2 = last20
    set1,set2=slidingWindow(s1,s2,2)
    dff=GetDiff(set1,set2)
    TSDexist=isTSD(dff,"DTT")
    conserve=Conserved(set1,dff,"DTT")
    if(conserve==True):
...
if __name__ == '__main__':
     for i in range(1,13):
         records = list(SeqIO.parse("chr%s_DTT_allCheckIRFHomo.fa" % (i),"fasta"))
         ...
         print(len(both))
         writeTofa("chr%s_DTT_allCheckIRFHomo.fa" % (i), "chr%s_DTT_irfhomo.fa" % (i),both,withTIR,"DTT")

#if __name__ == '__main__':
#    for i in range(1, 11):
#        records = list(SeqIO.parse("chr%s_DTT_allCheckPre.fa" % (i), "fasta"))
#        pool1 = multiprocessing.Pool(16)
#        L_tir = pool1.map(CheckTIR, records)
#        pool1.close()
#        pool1.join()
...
#        writeTofa("chr%s_DTT_allCheckPre.fa" % (i),"chr%s_irfml_DTT_withTIRTSD_Seq.fa" % (i))
#
#
