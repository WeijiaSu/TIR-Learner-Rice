...
def SplitFasta(contigFile,genomeFile,genomeName):
    ...
    l_name=list(names[0])
    for name in l_name:
        SeqIO.write((rec for rec in records if rec.id.split(":")[0]==name),"%s.fa"%(targetDir+genomeName+spliter+name),"fasta")
...
files=os.listdir(".")
files=[i for i in files if i.split(spliter)[0]==genome_Name and i[-3:]==".fa"]

for file in files:
    grf="%s/grf-main -i %s -o %s -c 1 -t %s -p 20 --min_space 10 " \
        "--max_space 10000 --max_indel 0 --min_tr 10 --min_spacer_len 10 --max_spacer_len 10000"%(grfP,file,file.split(spliter)[1][:-3],int(t))
    os.system(grf)
