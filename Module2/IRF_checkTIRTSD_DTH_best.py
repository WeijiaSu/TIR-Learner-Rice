...
if __name__ == '__main__':
     for i in range(1,13):
         records = list(SeqIO.parse("chr%s_DTH_allCheckIRFHomo.fa" % (i),"fasta"))
         ...
         print(len(both))
         writeTofa("chr%s_DTH_allCheckIRFHomo.fa" % (i), "chr%s_DTH_irfhomo.fa" % (i),both,withTIR,"DTH")

#if __name__ == '__main__':
#    for i in range(1, 11):
#        records = list(SeqIO.parse("chr%s_DTH_allCheckPre.fa" % (i), "fasta"))
#        pool1 = multiprocessing.Pool(16)
#        L_tir = pool1.map(CheckTIR, records)
#        pool1.close()
#        pool1.join()
...
#        writeTofa("chr%s_DTH_allCheckPre.fa" % (i), "chr%s_irfml_DTH_withTIRTSD_Seq.fa" % (i))
#
