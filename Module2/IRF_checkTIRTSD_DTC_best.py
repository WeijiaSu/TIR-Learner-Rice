...
def CheckTIR(rec):
    ...
    if (len(s)>20):
       ...
       for l in l_List:
           ...
           d = compare(s1, s2)
           if d < l*0.2 and Conserved(s1)==True:
               dic[str(rec.id)] = l
               withTIR.append(dic)
               break
       noTIR.append(dic)
...
if __name__ == '__main__':
     for i in range(1,13):
         records = list(SeqIO.parse("chr%s_DTC_allCheckIRFHomo.fa"%(i),"fasta"))
         ...
         print(len(both))
         writeTofa("chr%s_DTC_allCheckIRFHomo.fa"%(i),"chr%s_DTC_irfhomo.fa"%(i),both,withTIR,"DTC")


#if __name__ == '__main__':
#     for i in range(1, 11):
#         records = list(SeqIO.parse("chr%s_DTC_allCheckPre.fa" % (i), "fasta"))
#         pool1 = multiprocessing.Pool(16)
#         L_tir = pool1.map(CheckTIR, records)
#         pool1.close()
#         pool1.join()
...
#         writeTofa("chr%s_DTC_allCheckPre.fa" % (i),"chr%s_irfml_DTC_withTIRTSD_Seq.fa" % (i))
