...
def Conserved(wholeseq,set1,tsd_dffset,fam):
    for i in tsd_dffset:
        if (tsd_dffset[i]<TSD[fam]*0.2):
...
def CheckTSD(rec):
    ...
    conserve=Conserved(s,set1,dff,"DTT")
    if(conserve==True):
...
if __name__ == '__main__':
     for i in range(1,13):
         records = list(SeqIO.parse("DTT_fullcover_chr%s.fa"%(i),"fasta"))
         ...
         print(len(both))
         writeTofa("DTT_fullcover_chr%s.fa"%(i),"DTT_chr%s_fullcover_withTIR_best.fa"%(i),both,withTIR,"DTT")
         cat= "cat DTT_chr*_fullcover_withTIR_best.fa > DTT_fullcover_withTIR_best.fa"
         os.system(cat)
